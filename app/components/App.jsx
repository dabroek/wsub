...
class App extends React.Component {
  ...
  setFileAsLoaded(i, success) {
    this.setState({
      files: [
        ...
        Object.assign({}, this.state.files[i], {loading: false, success: success}),
  ...
  onDrop(files) {
    ...
    files.forEach((file, i) => {
      let s1 = addic7ed.search(file, LANG)
      let s2 = opensubtitles.search(file, LANG)
      let s3 = subscene.search(file, LANG)
  ...
  renderResults() {
    return this.state.files.map((file, i) => <div key={i} className="results__item">
      <span>{file.name}</span>
      <span>
        {(file.loading
          ? <Spinner color="white" config={{lines: 7, width: 2, radius: 2, length: 2}}/> 
          : (file.success ? <span>&#10003;</span> : <span>&#10007;</span>)
        )}
      </span>
    </div>)
  }
